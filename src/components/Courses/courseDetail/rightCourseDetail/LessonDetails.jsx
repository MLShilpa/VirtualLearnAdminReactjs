...
import {
  LessonType,
  SelectedFile,
} from "../../../../redux/reducers/createCourseSlice";
...
const LessonDetails = () => {
  ...
  useEffect(() => {
    setSelectedStatus(
      lessonData === ""
        ? ""
        : lessonData?.lessonDetails[0]?.lesson[0]?.type
        ? true
        : false
    );
    setSelectedType(
    ...
    setLessonName(
      lessonData === ""
        ? ""
        : lessonData?.lessonDetails[0]?.lesson[0]?.lessonName
    );
    setSelectedOption(
      lessonData === "" ? "" : lessonData?.lessonDetails[0]?.lesson[0]?.type
    );
    // setSelectedUrl(lessonData === "" ? "" : lessonData?.lessonDetails[0]?.lesson[0]?.link[0])
    setLessonId(
      lessonData === "" ? "" : lessonData?.lessonDetails[0]?.lesson[0]?._id
    );
  }, [lessonData]);
  ...
  const uploadVideoPreview = (e) => {
    ...
    fetch("https://api.cloudinary.com/v1_1/deiz877la/video/upload", {
      method: "post",
      body: data,
    })
      .then((res) => res.json())
      .then((data) => {
        ...
        data?.url &&
          setVideoFileName(data?.original_filename + "." + data?.format);
        setLoading(false);
        setLoadingMessage("");
      })
      .catch((err) => {
  ...
  const uploadFile = (e) => {
    ...
    fetch("https://api.cloudinary.com/v1_1/deiz877la/upload", {
      method: "post",
      body: data,
    })
      .then((res) => res.json())
      .then((data) => {
        ...
        if (selectedOption === "PDF") {
          ...
          data?.url &&
            setPdfFileName(data?.original_filename + "." + data?.format);
  ...
  const handleEdit = async (event) => {
    ...
    try {
      const fetchedData = await axios(`${Base_Url}/api/v1/edit_lesson`, {
        method: "patch",
        data: formData,
        headers: {
          Accept: "*/*",
                    ...
                    <div className="upload-dropDown">
                      ...
                      <div className="upload-videoTitle">
                        ...
                        >
                          ...
                          <option value="PDF" className="QandA-option">
                            PDF
                          </option>
                          <option value="PPT" className="QandA-option">
                            PPT
                          </option>
                          <option value="Video" className="QandA-option">
                            Video
                          </option>
                          <option value="Weblink" className="QandA-option">
                            Weblink
                          </option>
                        </select>
                      </div>
                            ...
                            <div>
                              <div className="PDFcontainer">
                                <label className="upload-title">
                                  Enter {selectedOption} URL
                                </label>
                                <br></br>
                                <input
                                  className="form-control"
                                  id={`{selectedOption}Url`}
                                  value={selectedWebUrl}
                                  onChange={(e) => {
                          ...
                          <>
                            <div>
                              <div className="upload-title">
                                Enter the type of {selectedOption}
                              </div>
                              <select
                                name="videoType"
                                className="upload-select"
                                ...
                                selectedType === "URL" && (
                                  <>
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        setSelectedPptUrl(e.target.value);
                                      }}
                                    />
                                  </>
                                )}
